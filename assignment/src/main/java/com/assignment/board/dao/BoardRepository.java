...
public interface BoardRepository extends JpaRepository<BoardVO, Integer> {
	...
	@Query(value = "SELECT b.boardNo FROM BoardVO b LEFT JOIN b.likes l " + "GROUP BY b.boardNo, b.createDate " + // <<<
																													// 여기에
																													// b.createDate
																													// 추가
			"ORDER BY COUNT(l) DESC, b.createDate DESC", countQuery = "SELECT COUNT(b) FROM BoardVO b")
	Page<Integer> findPopularBoardIds(Pageable pageable);

	List<BoardVO> findByParentBoardNoOrderByCreateDateAsc(Integer parentBoardNo);

	@Query("SELECT b FROM BoardVO b ORDER BY b.createDate DESC")
	Page<BoardVO> findAllPostsIgnoringParent(Pageable pageable);
