...
public class BoardService {
	...
	public List<BoardVO> getRecentPosts() {
		...
		if (!posts.isEmpty()) {
			populateLikeCounts(posts); // 중복 로직 대신 헬퍼 메서드 호출
		}
		return posts;
	}
	...
	public Page<BoardVO> getPopularBoardPage(Pageable pageable) {
		...
		populateLikeCounts(sortedPosts); // 중복 로직 대신 헬퍼 메서드 호출
    ...
    private void populateLikeCounts(List<BoardVO> posts) {
        ...
        // Repository를 통해 한번의 쿼리로 모든 좋아요 수를 가져옵니다.
        // 이 기능을 위해 LikeRepository에 findLikeCountsForBoardNos 메서드가 필요합니다.
        List<LikeCountDto> likeCounts = likeRepository.findLikeCountsForBoardNos(boardNos);
	...
	public Page<BoardVO> getBoardPageForTest(Pageable pageable) {
	    System.out.println("--- 테스트용 getBoardPageForTest 메소드 실행 ---");
	    // 1단계에서 만든 새로운 메소드를 호출합니다.
	    return boardRepository.findAllPostsIgnoringParent(pageable);
	}
}
