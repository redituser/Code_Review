...
public class CommentService {
    ...
    @Transactional
    public CommentVO updateComment(Integer commentNo, String newContent, String loginId) {
        // 1. 수정할 댓글을 DB에서 조회
        CommentVO comment = commentRepository.findById(commentNo)
                .orElseThrow(() -> new IllegalArgumentException("해당 댓글을 찾을 수 없습니다."));

        // 2. 권한 검사: 로그인한 사용자가 댓글 작성자인지 확인
        if (!comment.getWriterId().equals(loginId)) {
    ...
    @Transactional
    public void deleteComment(Integer commentNo, String loginId) {
        // 1. 삭제할 댓글을 DB에서 조회
        CommentVO comment = commentRepository.findById(commentNo)
                .orElseThrow(() -> new IllegalArgumentException("해당 댓글을 찾을 수 없습니다."));
        ...
        // 3. (선택사항) 대댓글이 있는 경우, 삭제 정책 결정
        // 예: "삭제된 댓글입니다"로 내용만 바꾸거나, 같이 삭제하거나 등등
        // 지금은 그냥 삭제 처리
